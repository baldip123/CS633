  ...
  start_time = MPI_Wtime ();
    ...
    for(int t = 0; t<Timesteps; ++t){
        ...
        if((myrow-1)>=0){
                ...
                MPI_Irecv(&top[0], 1, MPI_DOUBLE, get_rank(myrow - 1,mycol, P), myrank,
        ...
        if((myrow+1)<P){
            // there was an error here instead of P N was being used, both in the above check and the get rank function
                ...
                MPI_Irecv(&bottom[0], 1, MPI_DOUBLE, get_rank(myrow + 1,mycol, P), myrank,
        ...
        // sending to the top neg
        if((myrow-1)>=0){
            MPI_Wait (N, top_send_request, top_status);
            MPI_Wait (N, top_recv_request, top_status);  
        }
        //sending to the bottom neg
        if((myrow+1)<P){
            MPI_Wait (N, bottom_send_request, bottom_status);
            MPI_Wait (N, bottom_recv_request, bottom_status);  
        }
